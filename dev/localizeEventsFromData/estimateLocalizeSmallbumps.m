...
xb = linspace(1, data.Nx, round(data.Nx/(3*data.gridSize)));   % select x positions for basis
yb = linspace(1, data.Ny, round(data.Ny/(3*data.gridSize)));   % select y positions for basis

[xxb, yyb] = meshgrid(xb, yb);
decayRate = [10 10];                              % define decay rate of basis
